...
class GoodsMovement(Base):
    ...
    report_date: Mapped[date] = mapped_column(Date, nullable=False) # Дата отчета
    shop_sku: Mapped[str] = mapped_column(String(20), nullable=False) # SKU
    ...
    inventory_shortage: Mapped[int] = mapped_column(Integer, default=0) # Недостача при инвентаризации
    warehouse: Mapped[Optional[str]] = mapped_column(String(255), nullable=True) # Склад
